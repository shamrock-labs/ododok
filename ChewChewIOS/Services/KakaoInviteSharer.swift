...
/// 받은 사람이 메시지의 버튼을 누르면, 앱 설치 시 `code` 파라미터로 앱이 열려 자동 수락 흐름으로 이어진다.
...
enum KakaoInviteSharer {
    ...
    /// - Parameters:
    ///   - code: 내 초대 코드(서버 발급).
    ///   - deepLink: 서버가 준 공유 딥링크(chewchew://invite?code=...). 메시지 텍스트 보강용(선택).
    @MainActor
    static func share(code: String, deepLink: String?) async throws {
