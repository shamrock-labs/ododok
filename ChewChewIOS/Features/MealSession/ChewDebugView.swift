    ...
    private var parametersSection: some View {
        VStack(alignment: .leading, spacing: AppSpacing.three) {
            ...
            VStack(spacing: AppSpacing.four) {
                sliderRow("Peak Amplitude Cutoff", value: formatted(mealSession.chewSensitivity.minPeakAmplitude, digits: 3), binding: doubleBinding(\.minPeakAmplitude), range: 0.002...0.020)
                sliderRow("Minimum Peak Gap", value: "\(mealSession.chewSensitivity.minPeakGap) samples", binding: intBinding(\.minPeakGap), range: 8...50)
                sliderRow("Head Motion Cutoff", value: formatted(mealSession.chewSensitivity.headingMotionThreshold, digits: 2), binding: doubleBinding(\.headingMotionThreshold), range: 0.04...0.40)
                Divider()
                sliderRow("rotY Std Cutoff", value: formatted(mealSession.chewSensitivity.minimumRotationYStd, digits: 3), binding: doubleBinding(\.minimumRotationYStd), range: 0.005...0.100)
                sliderRow("Entry Duration", value: "\(mealSession.chewSensitivity.enterSampleCount) samples", binding: intBinding(\.enterSampleCount), range: 2...30)
                sliderRow("Exit Duration", value: "\(mealSession.chewSensitivity.exitSampleCount) samples", binding: intBinding(\.exitSampleCount), range: 2...150)
                Divider()
                sliderRow("rotY Dominance Cutoff", value: formatted(mealSession.chewSensitivity.minimumRotationYDominance, digits: 2), binding: doubleBinding(\.minimumRotationYDominance), range: 0...0.6)
                sliderRow("Jitter Dominance Cutoff", value: formatted(mealSession.chewSensitivity.minimumRotationYJitterBandDominance, digits: 2), binding: doubleBinding(\.minimumRotationYJitterBandDominance), range: 0...0.7)
                sliderRow("Accel / Rotation Cutoff", value: formatted(mealSession.chewSensitivity.maximumAccelToRotation, digits: 3), binding: doubleBinding(\.maximumAccelToRotation), range: 0.005...0.150)
            }
            .padding(AppSpacing.four)
            .background(Color.bgCard, in: RoundedRectangle(cornerRadius: AppRadius.container))
            .appElevation(.flat)
