...
리뷰 기준·등급·보고 형식은 `code-reviewer` 에이전트 정의(`.claude/agents/code-reviewer.md`)를 따른다. 코드 구조 규칙은 `CLAUDE.md`의 "코드 구조"를 정본으로 보고, iOS 도메인·상태·동시성 세부 루브릭은 `docs/ios-review-guide.md`를 함께 적용한다. 결과는 **P1(꼭 반영) / P2(적극 고려) / P3(사소)**로 등급을 매겨 파일·라인 근거와 함께 보고한다.
