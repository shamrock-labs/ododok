...
PR·diff를 리뷰할 때 쓰는 상세 루브릭. `CLAUDE.md`는 에이전트 압축 규칙, `README.md`는 프로젝트 진입점, `docs/ios-architecture.md`는 코드 작성 기준이고, 이 문서는 리뷰 절차다. 어떤 AI로 리뷰하든 이 파일을 붙여넣거나 링크해 기준을 맞춘다.
...
## 1. Store 계층 (기준 구현은 RecordsStore)
...
- **서버가 소유하는 값은 클라이언트가 재계산하지 않는다**(ODO-54). 서버 값은 반영만.
...
설계 기준: Obsidian `Projects/ododok/24 ios-domain-architecture-refactor.md`. 코드 작성 기준: `docs/ios-architecture.md`, `README.md`, `CLAUDE.md`.
