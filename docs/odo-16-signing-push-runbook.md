...
- `Project.swift` entitlements 설정 → Sign in with Apple + APNs `aps-environment`(Debug=development, Release=production)
- `tuist generate --no-open` + 시뮬레이터 무서명 빌드 SUCCEEDED
...
- 빌드 전 항상 `tuist generate --no-open` (이미 1회 실행됨)
- Signing & Capabilities: **Team 선택**(자동 서명), Push Notifications / Sign in with Apple capability 표시 확인
- 실기기 빌드 → 동작 확인
- 아카이브(Release): `APS_ENVIRONMENT=production` build setting으로 TestFlight/App Store용 APNs entitlement가 자동 적용됨
...
- **마켓 URL**에 App Store 앱 주소 `https://apps.apple.com/kr/app/ododok/id6784962920` 입력 → 미설치자가 앱스토어로 이동(미설치자 폴백)
- `KAKAO_INVITE_MOBILE_WEB_URL`도 같은 주소로 설정하면 카카오 템플릿 `mobileWebUrl` 폴백까지 같이 동작
  - xcconfig에서는 `//`가 주석 처리되므로 `https:/$()/apps.apple.com/kr/app/ododok/id6784962920`로 입력
- 코드 웹 폴백(`KakaoInviteSharer.Link.mobileWebUrl`)도 `KAKAO_INVITE_MOBILE_WEB_URL`이 설정된 경우 같이 적용됨
...
- [ ] 카카오 초대 링크: 미설치 기기에서 `https://apps.apple.com/kr/app/ododok/id6784962920`로 이동
