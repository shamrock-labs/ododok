# CD 세팅 가이드 (TestFlight / App Store Connect 업로드)

main에 머지하면 TestFlight로, `vX.Y.Z` 태그를 push하면 App Store Connect 업로드까지 자동으로 간다. TestFlight와 App Store 빌드는 모두 prod 백엔드를 사용한다. 심사 제출과 출시는 App Store Connect에서 사람이 한다.
...
| main push | `ios-cd-testflight.yml` | prod 백엔드 TestFlight 아카이브 → match 서명 → TestFlight 업로드 (버전 = `Config/Version.xcconfig`, 빌드번호 = App Store Connect 최신 + 1) |
| `vX.Y.Z` 태그 push | `ios-cd-release.yml` | prod Release 아카이브 → match 서명 → App Store Connect 업로드 (버전 = 태그, 빌드번호 = App Store Connect 최신 + 1, 심사 제출·출시는 수동) |
...
TestFlight 빌드도 App Store 빌드와 동일하게 `https://api.ododok.cloud`의 prod Spring·prod DB를 사용한다. dev Spring·dev DB 검증은 로컬 Debug 빌드에서 수행한다.

TestFlight의 사용자 노출 버전은 `Config/Version.xcconfig`의 `MARKETING_VERSION`을 기본값으로 쓴다. CD는 `CURRENT_PROJECT_VERSION`을 App Store Connect에 올라간 최신 빌드번호 + 1로 계산해 주입한다.

예를 들어 `MARKETING_VERSION = 1.0`이고 TestFlight/App Store Connect에 `1.0 (5)`가 이미 있으면, 다음 main 머지는 `1.0 (6)`을 올린다.

이미 App Store에 출시된 버전 train은 새 TestFlight 빌드를 받을 수 없다. CD는 App Store의 live 버전이 repo의 `MARKETING_VERSION` 이상이면 patch 버전을 자동으로 하나 올려 TestFlight에 주입한다. 예를 들어 repo가 `1.0.1`이고 App Store live 버전도 `1.0.1`이면, 다음 main 머지는 `1.0.2`로 TestFlight를 올린다.
...
기능 PR마다 `MARKETING_VERSION`을 바꾸지 않는다. 사용자가 보는 앱 버전을 명시적으로 올리고 싶을 때만 별도 PR로 `Config/Version.xcconfig`를 수정한다. 단, App Store에 이미 출시된 train과 충돌하는 경우에는 CD가 patch 버전을 자동으로 올린다.
...
버전 bump PR을 main에 머지하면 새 TestFlight version train이 열린다. 예를 들어 기존 `1.0 (8)` 다음에 `MARKETING_VERSION = 1.0.1`로 바꾸면 다음 빌드는 `1.0.1 (9)`처럼 올라갈 수 있다. 빌드번호가 1부터 다시 시작할 필요는 없다. 이후 `1.0.1`이 App Store에 출시되면 다음 TestFlight는 CD가 자동으로 `1.0.2`를 주입한다.

### App Store Connect 업로드와 제출
...
워크플로우는 prod 빌드를 App Store Connect에 업로드만 하고 심사를 제출하지 않는다. 업로드한 동일 빌드를 TestFlight에서 확인한 뒤 App Store Connect에서 그 빌드를 선택해 사람이 심사 제출한다. 심사 통과 후 최종 '출시'도 사람이 누른다.
...
- App Store Connect 업로드는 출시 버전 태그(`vX.Y.Z`)로 하고, 심사 제출은 동일 빌드의 TestFlight 검증 후 수동으로 한다.
